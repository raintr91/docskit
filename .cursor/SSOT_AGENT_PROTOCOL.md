...
>
> Tracker SSOT: `.harness/tasks/` (không dùng `.agents/tasks/` làm SSOT).
...
Dù Thread mới hay cũ, ngay khi User yêu cầu chạy một skill/tính năng, hành động **ĐẦU TIÊN BẮT BUỘC** là dùng tool `Read` (hoặc read-file tương đương) nhắm thẳng vào `SKILL.md` của skill đó để nạp lại quy chuẩn.

**TUYỆT ĐỐI KHÔNG** dựa vào trí nhớ mường tượng từ context/thread trước.

Chưa `Read` / read `SKILL.md` → **CẤM** tạo TODO, plan, hay bất kỳ file product nào.
...
`.harness/tasks/<tên-skill-hoặc-target>-todo.md`
...
`.harness/tasks/<tên-skill-hoặc-target>-plan.md`

Trong file này Agent **BẮT BUỘC**:
...
Chỉ khi file plan đã ghi xong toàn bộ → mới được quyền gọi `Write` / write tool cho product SSOT.

**TUYỆT ĐỐI KHÔNG** làm gộp. **TUYỆT ĐỐI KHÔNG** chỉ "nghĩ" plan trong chat/`<thought>` mà không ghi file plan.
...
**Ngoại lệ duy nhất:** Đề xuất Grill chưa được Member confirm — chỉ được ghi vào `.harness/tasks/*-proposal.md`, **TUYỆT ĐỐI KHÔNG** ghi đè product SSOT.
...
3. **Đề xuất:** 1 hoặc nhiều phương án; chỉ định **Recommended**. Ghi `.harness/tasks/*-proposal.md`.
4. **Hard Confirmation Gate:** Agent **TUYỆT ĐỐI KHÔNG** tự ý ghi đè đề xuất vào file product. Agent **BẮT BUỘC** dừng lại, hỏi Member. Chỉ sau khi Member **chốt / Confirm** mới được ghi SSOT.
...
```text
1) Read SKILL.md          → Đạo luật 1
2) write .harness/tasks/*-todo.md từ Workflow + Accelerators → Đạo luật 2
3) write .harness/tasks/*-plan.md (quote Verification Checklist) → Đạo luật 3
