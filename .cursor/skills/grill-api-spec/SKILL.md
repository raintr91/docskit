...
1. Resolve feature slug under `product/surfaces/<surface>/CMP-*/<slug>/` or `product/surfaces/common/yaml/<slug>/`; read spec and backend YAML trio
...
6. Run gates (repo root):
   `pnpm api:gen:dry --spec product/surfaces/<surface>/CMP-*/<slug>/01-backend-spec.yaml --write-spec` and `pnpm openapi:render`
